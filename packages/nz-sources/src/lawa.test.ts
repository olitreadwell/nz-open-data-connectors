...
import { fetchLawaRiverQualitySites, lawaAdapter, parseLawaRiverQualitySites } from './lawa';

const FIXTURE = JSON.parse(
  readFileSync(
    path.join(process.cwd(), 'src/fixtures/lawa-river-quality-sites-2026-08-25.json'),
    'utf8'
  )
) as unknown;
...
describe('parseLawaRiverQualitySites', () => {
  ...
  it('rejects a payload that is not a site list', () => {
    expect(() => parseLawaRiverQualitySites({ type: 'Feature' })).toThrow(NzSourceParseError);
  });
});
...
describe('fetchLawaRiverQualitySites', () => {
  ...
  it('throws an API error on a non-ok response', async () => {
    ...
    await expect(fetchLawaRiverQualitySites(fetchImpl)).rejects.toThrow(NzSourceApiError);
  });
});
