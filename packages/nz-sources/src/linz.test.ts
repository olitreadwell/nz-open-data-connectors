...
import { parseLinzLayers } from "./linz.js";

const FIXTURE = JSON.parse(
  readFileSync(
    path.join(process.cwd(), "src/fixtures/linz-layer-search.json"),
    "utf8",
  ),
) as unknown;
...
describe("parseLinzLayers", () => {
  it("parses the LINZ layer search fixture into layers", () => {
    ...
    const propertyTitles = layers.find(
      (layer) => layer.title === "NZ Property Titles",
    );
  ...
  it("rejects a payload that is not a layer search response", () => {
    expect(() => parseLinzLayers({ type: "FeatureCollection" })).toThrow(
      NzSourceParseError,
    );
  });
});
