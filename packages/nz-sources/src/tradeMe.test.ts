...
import { parseTradeMeCategories } from "./tradeMe.js";

const FIXTURE = JSON.parse(
  readFileSync(
    path.join(process.cwd(), "src/fixtures/trademe-categories.json"),
    "utf8",
  ),
) as unknown;
...
describe("parseTradeMeCategories", () => {
  ...
  it("rejects a payload without categories", () => {
    expect(() => parseTradeMeCategories({ Name: "Root" })).toThrow(
      NzSourceParseError,
    );
  });
});
