...
import { parseDataGovtDatastoreRows } from './dataGovtDatastore.js';
import { NzSourceParseError } from './errors.js';

const FIXTURE = JSON.parse(
  readFileSync(
    path.join(process.cwd(), 'src/fixtures/data-govt-datastore-msd-benefits.json'),
    'utf8'
  )
) as unknown;
...
describe('parseDataGovtDatastoreRows', () => {
  ...
  it('rejects a payload that is not a CKAN datastore response', () => {
    expect(() => parseDataGovtDatastoreRows({ success: false, error: {} })).toThrow(
      NzSourceParseError
    );
  });
});
