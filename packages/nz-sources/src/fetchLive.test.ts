...
describe("adapter fetchLive paths", () => {
  ...
  it("fetches and parses data.govt.nz datasets", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(readFixtureJson("data-govt-nz-search-sheep.json")),
      ),
    );
  ...
  it("fetches and parses DigitalNZ records", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(readFixtureJson("digitalnz-search-sheep.json")),
      ),
    );
  ...
  it("fetches and parses Trade Me categories", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(readFixtureJson("trademe-categories.json")),
      ),
    );
  ...
  it("fetches and parses LINZ layer search", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(readFixtureJson("linz-layer-search.json")),
      ),
    );
  ...
  it("fetches and parses data.govt.nz datastore rows", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(readFixtureJson("data-govt-datastore-msd-benefits.json")),
      ),
    );
  ...
  it("fetches and parses ADE search results", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(readFixtureJson("ade-search-earnings.json")),
      ),
    );
