...
const FIXTURE = JSON.parse(
  readFileSync(
    path.join(process.cwd(), 'src/fixtures/mfe-layer-search-water-2026-08-25.json'),
    'utf8'
  )
) as unknown;
...
describe('parseMfeLayers', () => {
  ...
  it('rejects a payload that is not a layer search response', () => {
    expect(() => parseMfeLayers({ type: 'FeatureCollection' })).toThrow(NzSourceParseError);
  });
});
...
describe('searchMfeLayers', () => {
  ...
  it('throws an API error on a non-ok response', async () => {
    ...
    await expect(searchMfeLayers('water', fetchImpl)).rejects.toThrow(NzSourceApiError);
  });
});
