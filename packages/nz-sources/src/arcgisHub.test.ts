...
describe("parseArcgisHubCollections", () => {
  it.each(COLLECTION_FIXTURES)(
    "parses the %s fixture into collections",
    (filename) => {
  ...
  it("rejects a payload without a collections list", () => {
    expect(() => parseArcgisHubCollections({ items: [] })).toThrow(
      NzSourceParseError,
    );
  });
});
...
describe("parseArcgisHubDatasets", () => {
  it("parses the Auckland parks fixture into datasets", () => {
    ...
    const parkExtents = datasets.find(
      (dataset) => dataset.title === "Park Extents",
    );
    expect(parkExtents?.source).toBe("Auckland Council");
  });

  it("rejects a payload without features", () => {
    expect(() => parseArcgisHubDatasets({ type: "FeatureCollection" })).toThrow(
      NzSourceParseError,
    );
  });
});
...
describe("arcgisHubAdapter", () => {
  ...
  it("passes host and fetchImpl through to the collections fetch", async () => {
    ...
    expect(calledUrl).toBe(
      "https://data-wcc.opendata.arcgis.com/api/search/v1/collections",
    );
  });
  ...
  it("parses a collections payload through the adapter", () => {
    const result = arcgisHubAdapter.parse(
      readFixture(COLLECTION_FIXTURES[0] ?? ""),
    );
    expect(result.kind).toBe("collections");
  });
...
describe("normalizeArcgisHubHost", () => {
  ...
  it("strips a trailing slash", () => {
    expect(
      normalizeArcgisHubHost("https://opendata.canterburymaps.govt.nz/"),
    ).toBe("https://opendata.canterburymaps.govt.nz");
  });
});
