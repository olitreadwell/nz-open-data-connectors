...
const FIXTURE = JSON.parse(
  readFileSync(
    path.join(process.cwd(), 'src/fixtures/lris-layer-search-soil-2026-08-25.json'),
    'utf8'
  )
) as unknown;
...
describe('parseLrisLayers', () => {
  ...
  it('rejects a payload that is not a layer search response', () => {
    expect(() => parseLrisLayers({ type: 'FeatureCollection' })).toThrow(NzSourceParseError);
  });
});
...
describe('searchLrisLayers', () => {
  ...
  it('throws an API error on a non-ok response', async () => {
    ...
    await expect(searchLrisLayers('soil', fetchImpl)).rejects.toThrow(NzSourceApiError);
  });
});
