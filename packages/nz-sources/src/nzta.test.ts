...
import { fetchNztaHolidayHotspots, nztaAdapter, parseNztaHolidayHotspots } from './nzta';

const FIXTURE = JSON.parse(
  readFileSync(
    path.join(process.cwd(), 'src/fixtures/nzta-holiday-hotspots-2026-08-25.json'),
    'utf8'
  )
) as unknown;
...
describe('parseNztaHolidayHotspots', () => {
  ...
  it('rejects a payload that is not a hotspot map', () => {
    expect(() => parseNztaHolidayHotspots({ type: 'Feature' })).toThrow(NzSourceParseError);
  });
});
...
describe('fetchNztaHolidayHotspots', () => {
  ...
  it('throws an API error on a non-ok response', async () => {
    ...
    await expect(fetchNztaHolidayHotspots(fetchImpl)).rejects.toThrow(NzSourceApiError);
  });
});
