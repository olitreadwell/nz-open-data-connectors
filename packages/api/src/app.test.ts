...
function createStubStatsNzClient(): StatsNzClient {
  return {
    getDataflowCatalogue: async () => [
      { id: 'AGR_AGR_003', agencyId: 'STATSNZ', version: '1.0', title: 'Agricultural production' },
    ],
...
describe('createConnectorsApp', () => {
  it('answers the health check', async () => {
    ...
    await expect(res.json()).resolves.toEqual({ ok: true, name: 'nz-open-data-connectors' });
  });
  ...
  it('returns data rows as JSON by default', async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { dataflowId: string; rows: Array<{ value: number }> };
  ...
  it('returns data rows as CSV when format=csv', async () => {
    const app = createTestApp();
    const res = await app.request('/api/stats-nz/data?dataflowId=AGR_AGR_003&format=csv');
    expect(res.status).toBe(200);
